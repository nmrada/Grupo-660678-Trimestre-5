...
calls to this DAO, otherwise a new Connection will be allocated for each operation.
	...
	public ProveedorPk insert(Proveedor dto) throws ProveedorDaoException
	{
		...
		try {
			...
			if (dto.isTipoDeIdentificacionModified()) {
				if (modifiedCount>0) {
					sql.append( ", " );
					values.append( ", " );
				}
		
				sql.append( "tipoDeIdentificacion" );
				values.append( "?" );
				modifiedCount++;
			}
	...
	public void update(ProveedorPk pk, Proveedor dto) throws ProveedorDaoException
	{
		...
		try {
			...
			sql.append( " WHERE idProveedor=? AND tipoDeIdentificacion=?" );
			if (logger.isDebugEnabled()) {
				logger.debug( "Executing " + sql.toString() + " with values: " + dto);
			}
			...
			stmt.setString( index++, pk.getIdProveedor() );
			stmt.setString( index++, pk.getTipoDeIdentificacion() );
			int rows = stmt.executeUpdate();
			reset(dto);
			long t2 = System.currentTimeMillis();
			if (logger.isDebugEnabled()) {
	...
	protected void reset(Proveedor dto)
	{
		...
		dto.setTipoDeIdentificacionModified( false );
